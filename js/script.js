...
for (let i = 1; i < sectionCount; i++) {
  document.querySelector('.indicator').innerHTML += '<li><a href class="btn"></a></li>\n';
}
...
indicators.forEach((indicator, i) => {
  indicator.addEventListener('click', e => {
    ...
    e.stopPropagation()
    setAriaPressed(i)
		const scrollTop = Math.floor(scroller.scrollHeight * (i / sectionCount))
    smoothScroll(scroller, scrollTop, false)
  })
})

scroller.addEventListener('scroll', debounce(() => {
  let index = Math.round((scroller.scrollTop / scroller.scrollHeight) * sectionCount);
  setAriaPressed(index);
  if(scroller.scrollTop >= window.innerHeight / 2){
    navbar.classList.add("not-at-top");
  }else{
    navbar.classList.remove("not-at-top");
  }
}, 200))
